...
router.post('/anycast-plan',[
  ...
  function (req,res,next) {
	  ...
	  if (!errors.isEmpty() || req.recaptcha.error) {
		  ...
		  let full_name = req.body.full_name;
		  let company_textarea = req.body.contact_textarea;
		  let anycast_plan = req.body.anycast_plan;

		  let data = {
			  email: req.body.email,
			  full_name: req.body.full_name,
			  company_textarea: req.body.company_textarea,
			  anycast_plan:req.body.anycast_plan,
			  ip: req.header('X-Real-IP') || req.connection.remoteAddress
		  };
		  let transporter = node_mailer.createTransport({
			  host: 'mail.vying.ro',
		  ...
		  let transform = {
			  '<>': 'h3', 'html': function () {
				  return ('<h3>E-mail: </h3>' + this.email + '<h3>Numele: </h3>' + this.full_name  + '<h3>Datele companiei: </h3>' + this.company_textarea + '<h3>Tipul de plan: </h3>' + this.anycast_plan + '<h3>Ip-ul user-ului este: </h3>' + this.ip);
			  }
		  };
		  let html = json2html.transform(data, transform);
		  let mailOptions = {
...
router.post('/unicast-plan',[
  ...
  function (req,res,next) {
	  ...
	  if (!errors.isEmpty() || req.recaptcha.error) {
		  ...
		  let full_name = req.body.full_name;
		  let company_textarea = req.body.contact_textarea;
		  let unicast_plan = req.body.unicast_plan;

		  let data = {
			  email: req.body.email,
			  full_name: req.body.full_name,
			  company_textarea: req.body.company_textarea,
			  unicast_plan:req.body.unicast_plan,
			  ip: req.header('X-Real-IP') || req.connection.remoteAddress
		  };
		  let transporter = node_mailer.createTransport({
			  host: 'mail.vying.ro',
		  ...
		  let transform = {
			  '<>': 'h3', 'html': function () {
				  return ('<h3>E-mail: </h3>' + this.email + '<h3>Numele: </h3>' + this.full_name  + '<h3>Datele companiei: </h3>' + this.company_textarea + '<h3>Tipul de plan: </h3>' + this.unicast_plan + '<h3>Ip-ul user-ului este: </h3>' + this.ip);
			  }
		  };
		  let html = json2html.transform(data, transform);
		  let mailOptions = {
